(function(){
  ...
  if(!document.querySelector('.kennel-screen')){
    ...
    if(oldKennel){
      const nodes={};
      ['kennelBase','rosterPanel','creatorPanel','openCreatorBtn','recoveryPanel','hubTraderGrid','marketPanel','upgrades','questGrid','perkGrid','researchGrid'].forEach(id=>{ nodes[id]=$(id); });
      ...
      kennel.innerHTML=`
        <header class="kennel-header panel">
          <div><p class="eyebrow">Kennel command</p><h2>Pack Headquarters</h2><p class="muted">Manage the pack, build supplies, trade, and plan the next run.</p></div>
        ...
        <div class="kennel-view active" data-kennel-panel="pack">
          <article class="panel kennel-card kennel-roster-card"><div class="panel-title-row"><div><h3>Raiders</h3><p class="muted small">Your active pack and new recruits.</p></div><span data-slot="openCreatorBtn"></span></div><div data-slot="rosterPanel"></div><div data-slot="creatorPanel"></div></article>
        ...
        <div class="kennel-view" data-kennel-panel="tasks"><article class="panel kennel-card kennel-wide-card"><h3>Noticeboard</h3><div data-slot="questGrid"></div></article></div>`;
      Object.entries(nodes).forEach(([id,node])=>{ const slot=kennel.querySelector(`[data-slot="${id}"]`); if(slot&&node) slot.replaceWith(node); });
